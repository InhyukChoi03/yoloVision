...
- 탐지 모듈
  ...
  - `Features/Detection/DetectionService.swift`:
    ...
    - 한국어 라벨 매핑
    - 보행 우선 객체 선택 + 방향(왼쪽/정면/오른쪽) 음성 안내 트리거
    - 라벨별 안내 쿨다운(반복 알림 완화)

- 음성 모듈
  - `Features/Speech/SpeechService.swift`:
    - 한국어(ko-KR) `AVSpeechSynthesizer` 직렬 발화
    - 최소 발화 간격 / 중복 문장 억제 / 진행 중 무시(겹침 방지)
    - `.playback` + `.voicePrompt`(duck) 오디오 세션 구성
...
- 현재 미구현/보완 필요
  - 탐지 후처리 고도화(NMS 후 안정화 규칙, 다중 객체 동시 안내 정책)
  - 거리/근접도 추정 기반 우선순위(현재는 화면 내 방향만 안내)
...
### Phase 3. 한국어 TTS 파이프라인 (구현 완료)
- 목표: 안내 음성 품질 확보
- 작업:
  - `SpeechService` 구현(ko-KR) — 완료
  - 최소 발화 간격/중복 문장 억제/직렬 처리 — 완료
  - 인터럽트 후 복구(force 발화 시 즉시 중단/재생) — 완료
- 완료 기준:
  - 겹침 없이 자연스러운 안내 — 충족
